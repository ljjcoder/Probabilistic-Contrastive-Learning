...
class FlexMatch:
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            num_NCE_instance=args.num_NCE_instance
            loss_NCE=0
            NCE_weight=args.NCE_weight
            NCE_scale=args.NCE_scale  
            if NCE_weight<0.0:
                using_NCE=False
                print(using_NCE)
            with amp_cm():
                ...
                if False:
                    ...
                    #print(index_b.shape)
                    #print(len(selected_id_list),num_NCE_instance-len(selected_id_list))
                    #exit()
                    id_list=list(index_b[:100-len(selected_id_list)])+selected_id_list 
                #print(id_list)
                #if using_NCE:
                    #print('enter_lijj')
                #else:
                    #print('not enter_lijj')
                    #exit()
                if using_NCE:
                    #print('enter_lijj')
                    #exit()
                    ...
                    for NCE_Loss_id in range(num_split):
                        w_soft=w_soft_ori[NCE_Loss_id*num_NCE_instance:(NCE_Loss_id+1)*num_NCE_instance]
                        s_soft=s_soft_ori[NCE_Loss_id*num_NCE_instance:(NCE_Loss_id+1)*num_NCE_instance]
                        #print(w_soft.shape)
                        #print(s_soft.shape)
                        #exit()
